    ...
    io.on('connection', function(socket) {
      ...
      new Promise(function (resolve) {
        ...
        function debugStart(data) {
          ...
          if(suffixIndex<0){
            return socket.emit('debug.start', {success:false,desc:'no suffix'})
          }
          if(scriptName.substring(suffixIndex+1)!='py'){
            return socket.emit('debug.start', {success:false,desc:'only support python script'})
          }
          //发送禁止屏幕响应的事件
          ...
          fileutil.mkdirs(resPath)
            .then(function(){
              var makeIMG = new Promise(function (resolve, reject) {
                fs.writeFile(resPath + '/' + data.img.name,data.img.data, function (err) {
                  if (err) {
                    reject(err);
                  } else {
                    resolve();
      ...
      var appiumServer = http.createServer(function (req, res) {
        ...
        req.on('end', function () {
          ...
          console.log(bodyObj.params.message)
          socket.emit('debug.log',{success:true,data:bodyObj.params.message})
        })
      })
