      ...
      new Promise(function (resolve) {
        ...
        socket.on('disconnect', resolve)
          ...
          .on('image.match',function(data) {
            ...
            //图片生成到本地，
            var resPath = screenDir+'/'+data.name;
            console.log(resPath)
            ...
            var pyPath = pathutil.root('python_lib/find_obj.py')
            var child = proChild.exec('python ' + pyPath + ' ' + resPath + ' ' + screenDir+'/screen.png', function (err, stdout, stderr) {
              ...
              if (i != -1) {
                ...
                data.result = sub.substring(0, sub.indexOf('inliers/matched')).replace(/\s/ig, '');
              }
              socket.emit('image.match.return', data)
            })
          })
          //.on('image.match.return',function)
          //截图完毕，清除截图缓存
          .on('screenshot.end',function(){
            console.log('screenshot.end:',screenDir)
            //删除screenPath指定的文件
            if(screenDir!=''){
              fileutil.rmFilesByPath(screenDir)
                .then(function(){
                  fs.rmdir(screenDir,function(err){
                    console.log(err)
                  });
                })
            }
              ...
              var img = new Promise(function (resolve, reject) {
                adb.screencap(options.serial)
                  ...
                  .then(function (screencap) {
                    resolve(screencap);
                    //将截图生成到本地，用于图像比对
                    screenDir=pathutil.root('script_debug')+'/'+Date.now().toString()
                    fileutil.mkdirs(screenDir)
                      .then(function(){
                        fs.writeFile(screenDir+'/screen.png', screencap, function(err) {
