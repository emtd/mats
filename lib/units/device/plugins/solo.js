...
module.exports = syrup.serial()
  ...
  .dependency(require('./util/msgwsoption'))
  .define(function(options, sub, push, router, identity,msgWSOption) {
    ...
    router.on(wire.ProbeMessage, function() {
      var msgUrl,proxyMsgUrl;
      if(options.proxyMode){
        msgUrl='';//会在group.invite中重新更新值
        proxyMsgUrl=msgWSOption.msgWSUrl
      }else{
        msgUrl=msgWSOption.msgWSUrl
        proxyMsgUrl=''
      }
