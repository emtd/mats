  ...
  .define(function(options,push,router,identity,dsc,logcat) {
    ...
    router
      .on(wire.ScriptStartMessage, function(channel, message) {
        ...
        Promise.all([getAppium(),mkDir])
          .spread(function(){
            runPython(false, JSON.parse(message.env), capPath, devLog, scriptsPath,message)
          })
      })
    ...
    io.on('connection', function (socket){
      ...
      socket.on('disconnect', function(){
        ...
        .on('debug.capture',function(data){
          console.log('--------------debug.capture:',data)
          /*{
            user:'703'
            ,img:[]
          }*/
          if (!data||!data.user||!data.img||data.img.constructor.name!='Array'){
            console.log('return')
            return;
          }
          var dir = pathutil.root(process.env.debugPath + '/' + data.user + '/' + options.serial + '/capture')
          console.log('dir:',dir)
          if (fs.existsSync(dir)) {
            fs.readdir(dir, function (err, files) {
              //console.log(files)
              var temp=_.difference(files,data.img);
              //console.log(temp)
              if(temp.length>0){
                var promiseList=[]
                ...
                Promise.all(promiseList)
                  .then(function(data){
                    socket.emit('debug.capture.return', data)
                  })
              }
            })
          }
    ...
    function runPython(isDebug,envData,capPath,devLogPath,scriptPath,message) {
      ...
      var logPath = pathutil.logPath(pathobj.datePath, options.serial, process.env.logName);
      var perPath = pathutil.logPath(pathobj.datePath, options.serial, process.env.perName);
      var logdir=logPath+'/'+pathobj.logName;
      var perdir=perPath+'/'+pathobj.perName;
      logcat.start([],message.packageName,logdir,perdir)
        ...
        child = proChild.exec(envStr + 'python ' + scriptPath, function (err, stdout, stderr) {
          ...
          if(isDebug){//脚本调试
            ...
            var testResult = {
              ...
              task_id: message.taskId,//任务ID
              device_log: devLogPath,
              reportcontent: ''//,结果描述,富文本
            }
            var dataStr = 'data/'
            testResult.tasklogurl = logdir.substring(logdir.indexOf(dataStr) + dataStr.length)
            testResult.taskperformanceurl = perdir.substring(perdir.indexOf(dataStr) + dataStr.length)
            ...
            var setCapPath=new Promise(function(resolve,reject){
              //设置截图路径:
              fs.readdir(capPath, function (err, files) {
                ...
                var text=dateutil.timeToStr()+',script:\r\n'+JSON.stringify(testResult)+'\r\n';
                fileutil.endWrite(pathutil.root('log/'+dateutil.dateToStr()+'/'+new Date().getHours().toString()),text)
                resolve();
              })
            })

            var setDevLogPath=new Promise(function(resolve,reject){
              fs.readdir(devLogPath, function (err, files) {
                if (!files || files.length == 0) {
            ...
            Promise.all([setCapPath,setDevLogPath])
              .then(function(){
                push.send([
                  wireutil.global
