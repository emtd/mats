...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, router, push, group) {
          ...
          return plugin.stop()
            .then(function() {
              ...
              time = setInterval(function () {
                adb.openPerformanceInfo(options.serial)
            ...
            .then(function(logcat) {
              ...
              function entryListener(entry) {
                ...
                var split = '|#|'
                var data = [entry.priority, timestr, entry.pid, entry.tid, null, entry.tag, entry.message.replace('\n', '<br />')]
    ...
    plugin.stop = Promise.method(function() {
      ...
      if(logdir){
        fileutil.endWrite(logdir);
      }
      if(perdir){
        fileutil.endWrite(perdir);
      }
      if (plugin.isRunning()) {
    ...
    router
      .on(wire.LogcatStartMessage, function(channel, message) {
        console.log('----------------------logcat receive LogcatStartMessage:',message)
        var reply = wireutil.reply(options.serial)
        if (!message || !message.datePath || !message.logName || !message.performanceName) {
          console.error('--------------------logcat argument error')
          return;
        }
        ...
        console.log('------------------------perpath:', perPath)
        logdir=logPath+'/'+logName;
        perdir=perPath+'/'+perName;
