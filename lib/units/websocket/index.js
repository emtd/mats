              ...
              return new Promise(function(resolve,reject){
                dbapi.loadDevice(serial)
                  .then(function(dev){
                    //screen proxy
                    var screenProxy={};
                    screenProxy.url=dev.display.screenUrl;
                    if(dev.display.url==''){
                      makeScreenProxyPort()
                    }
                    else{
                      var i=dev.display.url.lastIndexOf(':')
                      if(i!=-1){
                        screenProxy.port=dev.display.url.substring(i+1)
                      }else{
                        makeScreenProxyPort()
                      }
                    }
                    ...
                    //msg proxy
                    var msgProxy={};
                    msgProxy.url=dev.proxyMsgUrl
                    if(dev.msgWSUrl==''){
                      makeMsgProxyPort()
                    }
                    else{
                      var i=dev.msgWSUrl.lastIndexOf(':')
                      if(i!=-1){
                        msgProxy.port=dev.msgWSUrl.substring(i+1)
                      }else{
                        makeMsgProxyPort()
                      }
                    }
                    ...
                    function getIp(ip){
                      return process.env.IPMAP[ip]?process.env.IPMAP[ip]:ip
                    }
                  })
              })
            })
                ...
                .spread(function (ioServer, ioClient) {
                  ...
                  sockList[serial].msg['client']
                    .on('disconnect', function () {
                      ...
                      if (sockList[serial].msg['server'].conn) {
                        sockList[serial].msg['server'].disconnect()
                        delete sockList[serial].msg['server'];
                      }
