  ...
  io.on('connection', function(socket) {
    ...
    var messageListener = wirerouter()
      ...
      .on(wire.JoinGroupByAdbFingerprintMessage, function(channel, message) {
        log.info('send JoinGroupByAdbFingerprintMessage----user.keys.adb.confirm:',message.comment,message.fingerprint)
      ...
      .on(wire.DeviceIdentityMessage, function(channel, message) {
        log.info('send DeviceIdentityMessage:')
      ...
      .on(wire.TransactionDoneMessage, function(channel, message) {
        //log.info('send TransactionDoneMessage:')
      ...
      .on(wire.PerformanceEntryMessage,function(channel,message){
        log.info('send PerformanceEntryMessage');
        socket.emit('performance.entry', message)
      })
      .on(wire.ScriptAutoEndMessage,function(channel,message){
        log.info('send ScriptAutoEndMessage');
        socket.emit('device.change', {
         important: true
         , data: {
         serial: message.serial
    ...
    new Promise(function(resolve) {
      socket.on('disconnect', resolve)
        ...
        .on('user.keys.adb.add', function(data) {
          log.info('receive user.keys.adb.add:',data)
        ...
        .on('user.keys.adb.accept', function(data) {
          log.info('receive user.keys.adb.accept:',data)
        ...
        .on('user.keys.adb.remove', function(data) {
          log.info('receive user.keys.adb.remove:',data)
        ...
        .on('group.invite', function(channel, responseChannel, data) {
          log.info('receive group.invite')
          var serial=data.requirements.serial.value;
          dbapi.loadDevice(serial)
            .then(function(dev){
              log.info('token match:',dev.token,user.email)
              return new Promise(function(resolve,reject){
                if(dev!=null&&dev.token==user.email){
                  joinChannel(responseChannel)
                  //非代理模式，则直接通信并返回
                  if (!data.proxy){
                    push.send([
              ...
              return new Promise(function(resolve,reject){
                dbapi.loadDevice(serial)
                  .then(function(dev){
                    //screen proxy
                    var screenProxy={};
                    screenProxy.url=dev.display.url;
                    if(dev.display.proxyUrl==''){
                      makeScreenProxyPort()
                    }
                    else{
                      var i=dev.display.proxyUrl.lastIndexOf(':')
                      if(i!=-1){
                        screenProxy.port=dev.display.proxyUrl.substring(i+1)
                      }else{
                        makeScreenProxyPort()
                      }
                    }
                    ...
                    //msg proxy
                    var msgProxy={};
                    msgProxy.url=dev.msgUrl
                    if(dev.proxyMsgUrl==''){
                      makeMsgProxyPort()
                    }
                    else{
                      var i=dev.proxyMsgUrl.lastIndexOf(':')
                      if(i!=-1){
                        msgProxy.port=dev.proxyMsgUrl.substring(i+1)
                      }else{
                        makeMsgProxyPort()
                      }
                    }
                    resolve([screenProxy,msgProxy])

                    function makeScreenProxyPort(){
                      screenProxy.port=options.proxyPorts.shift();
                      var url='ws://'+getIp(require('my-local-ip')())+':'+screenProxy.port
                      dbapi.setDeviceDisUrl(serial,url)
                    }

                    function makeMsgProxyPort(){
                      msgProxy.port=options.proxyPorts.shift();
                      var url='http://'+getIp(require('my-local-ip')())+':'+msgProxy.port
                      dbapi.setDeviceMsgProxyUrl(serial,url)
                    }

                    function getIp(ip){
                      var map=JSON.parse(process.env.IPMAP)
                      return map[ip]?map[ip]:ip
                    }
                  })
              })
            })
            .then(function(data) {
              console.log('--------------------',data)
              var screenProxy = data[0];
              var msgProxy = data[1];

              if (!sockList[serial]) {
                sockList[serial] = {screen: Object.create(null), msg: Object.create(null)}
              }
              var getScreenServer, getScreenClient, getMsgServer, getMsgClient;

                //screen server
                getScreenServer = new Promise(function (resolve, reject) {
                  if (sockList[serial].screen['server']) {
                    resolve();
                  }
                  else{
                    if(!sockList[serial].screen['wss']){
                      sockList[serial].screen['wss']=new WebSocket.createServer({port: screenProxy.port, perMessageDeflate: false});
                      sockList[serial].screen['port'] = screenProxy.port;
                    }
                    //log.info(serial+' server on '+screenProxy.port)
                    function proxyListener() {
                    }

                    function proxyError(err) {
                      //关闭所有跟代理服务有关的连接，关闭和screen的socket连接
                    }

                    sockList[serial].screen['wss']
                    .on('listening', proxyListener)
                    .on('error', proxyError)
                    .on('connection', function (wsServer) {
                      sockList[serial].screen['server'] = wsServer;
                      resolve()
                    })
                  }
                })
                ...
                //screen client
                getScreenClient = new Promise(function (resolve, reject) {
                  if(sockList[serial].screen['client']){
                    resolve()
                  }
                  else{
                    sockList[serial].screen['client']=new WebSocket(screenProxy.url);
                    sockList[serial].screen['client']
                      .on('open', function () {
                      resolve()
                    })
                    .on('error', function (err) {
                      if(sockList[serial].screen['client']){
                        sockList[serial].screen['client'].close(1001,'socket error')
                      }
                    })
                    .on('close', function (code, reason) {
                      if(sockList[serial]){
                        if(sockList[serial].screen){
                          if(sockList[serial].screen['wss']){
                            sockList[serial].screen['wss'].close();
                          }
                        }
                      }
                    })
              ...
              Promise.all([getScreenServer, getScreenClient])
                .spread(function (wsServer, wsClient) {
                  //此处应该保证socklist中用到的值都不为空
                  sockList[serial].screen['server']
                    .on('message', function (data) {
                      if(sockList[serial].screen['client']){
                        if (sockList[serial].screen['client'].readyState == WebSocket.OPEN) {//已经建立连接，直接发送
                          sockList[serial].screen['client'].send(data);
                        } else {//没有建立连接，放入数组，等待建立连接后发送
                          //serverMsg.push(data);
                          // log.info('wsClient readyState:', wsClient.readyState)
                        }
                      }
                    })
                    .on('close', function (data) {
                      if(sockList[serial]){
                        if(sockList[serial].screen){
                          if(sockList[serial].screen['server']){
                            delete sockList[serial].screen['server'];

                            //去掉client的监听,否则代理server断开后，代理client还在接受数据。
                            //sockList[serial].screen['client'].removeListener('message',clientMessage);
                            //sockList[serial].screen['client'].removeListener('close',clientClose);
                          }
                        }
                      }
                  ...
                  sockList[serial].screen['client']
                    ...
                    .on('close', clientClose)


                  function clientMessage(data){
                    if(sockList[serial].screen['server']){
                      if (sockList[serial].screen['server'].readyState == WebSocket.OPEN) {
                        sockList[serial].screen['server'].send(data);
                      }
                    }
                  }
                  function clientClose(code,reason){
                    //log.info('screen websocket for ' + serial + ' is close for reason:' + reason)
                    if(sockList[serial]){
                      if(sockList[serial].screen){
                        if(sockList[serial].screen['wss']){
                          sockList[serial].screen['wss'].close(function(){
                            delete sockList[serial].screen['wss'];
                          });
                        }
                        if(sockList[serial].screen['client']){
                          delete sockList[serial].screen['client'];
                        }
                      }
                    }
                  }
              ...
              getMsgServer = new Promise(function (resolve, reject) {
                if (sockList[serial].msg['server']) {
                  resolve(sockList[serial].msg['server'])
                }
                else {
                  if(!sockList[serial].msg['wss']){
                    sockList[serial].msg['wss']=socketio();
                    sockList[serial].msg['wss'].listen(msgProxy.port);
                  }
                  sockList[serial].msg['wss']
                    .on('connect', function (socket) {
                      sockList[serial].msg['server'] = socket;
                      resolve()
                    })

                  sockList[serial].msg['port'] = msgProxy.port;
                }
              })
                .catch(function (err) {
                  log.info('MsgServer error:', err)
                })

              getMsgClient = new Promise(function (resolve, reject) {
                if (sockList[serial].msg['client']) {
                  resolve()
                }
                else {
                  sockList[serial].msg['client'] = socketio_client(msgProxy.url, {
                    reconnection: false,
                    transports: ['websocket']
                  })//http://192.168.27.65:7110
                  sockList[serial].msg['client']
                    .on('connect', function () {
                      sockList[serial].msg['client'].emit('user', user)
                      resolve()
                    })
                }
                ...
                .catch(function (err) {
                  log.info('getMsgClient error:', err)
                })

              Promise.all([getMsgServer, getMsgClient])
                .spread(function (ioServer, ioClient) {
                  sockList[serial].msg['server']
                    .on('disconnect', function (data) {
                      if (sockList[serial].msg['server']) {
                        delete sockList[serial].msg['server']
                      }
                      sockList[serial].msg['client'].removeListener()
                    })
                    .on('error', function (data) {
                      /*sockList[serial].msg['server'].disconnect();
                      if (sockList[serial].msg['server']) {
                        delete sockList[serial].msg['server']
                      }*/
                    })
                    .on('debug.start', function (data) {
                      sockList[serial].msg['client'].emit('debug.start', data)
                    })
                    .on('debug.appui', function (data) {
                      sockList[serial].msg['client'].emit('debug.appui', data)
                    })
                    .on('debug.stop', function (data) {
                      sockList[serial].msg['client'].emit('debug.stop', data)
                    })
                    .on('image.match', function (data) {
                      sockList[serial].msg['client'].emit('image.match', data)
                    })
                    .on('screenshot.end', function (data) {
                      sockList[serial].msg['client'].emit('screenshot.end', data)
                    })

                  sockList[serial].msg['client']
                    .on('disconnect', function () {
                      if (sockList[serial].msg['wss']) {
                        sockList[serial].msg['wss'].close();
                        delete sockList[serial].msg['wss'];
                        delete sockList[serial].msg['port'];
                      }
                      if (sockList[serial].msg['server']) {
                        sockList[serial].msg['server'].disconnect()
                        delete sockList[serial].msg['server'];
                      }
                      delete sockList[serial].msg['client'];
                    })
                    .on('connect_error', function () {
                      log.info(serial, 'connect_error');
                    })
                    .on('debug.stop.return', function (data) {
                      sockList[serial].msg['server'].emit('debug.stop.return', data)
                    })
                    .on('debug.stop.return', function (data) {
                      sockList[serial].msg['server'].emit('debug.stop.return', data)
                    })
                    .on('image.match.return', function (data) {
                      sockList[serial].msg['server'].emit('image.match.return', data)
                    })
                    .on('debug.appui', function (data) {
                      sockList[serial].msg['server'].emit('debug.appui', data)
                    })
                    .on('debug.start.return', function (data) {
                      sockList[serial].msg['server'].emit('debug.start.return', data)
                    })
                    .on('debug.log', function (data) {
                      sockList[serial].msg['server'].emit('debug.log', data)
                    })
                    .on('socket.ip', function (data) {
                      sockList[serial].msg['server'].emit('socket.ip', data)
                    })
                    .on('connect_timeout', function () {
                      log.info(serial + ' connect_timeout');
                    })
                    .on('reconnect', function (num) {
                      log.info('msg wsClient reconnect:' + num);
                    })
                    //断开重连
                    .on('error', function (err) {
                      //ioClient.open()
            ...
            .catch(function(err){
              log.error('group.invite error:',err)
            })
        })
        .on('group.kick', function(channel, responseChannel, data) {
          log.info('receive group.kick:',data)
          var serial=data.requirements.serial.value;
          dbapi.loadDevice(serial)
            .then(function(dev){
              if(dev!=null&&dev.token==user.email){
          ...
          //if(options.proxyMode){
            dbapi.setDeviceDisUrl(data.requirements.serial.value,'');
            dbapi.setDeviceMsgProxyUrl(data.requirements.serial.value,'');
            var sock=sockList[data.requirements.serial.value]
            if(sock){
              if(sock.screen){
                if(sock.screen.wss){
                  sock.screen.wss.close(function(){
                    log.info(data.requirements.serial.value,' wss closed')
                  })
                }
                if(sock.screen.server){
                  sock.screen.server.close(1000,'group.leave')
                }
                if(sock.screen.client){
                  sock.screen.client.close(1000,'group.leave')
                }
                if(sock.screen.port){
                  options.proxyPorts.unshift(sock.screen.port)
                }
              }
              if(sock.msg){
                if(sock.msg.wss){
                  sock.msg.wss.close(function(){
                    log.info(data.requirements.serial.value,' wss closed')
                  })
                }
                if(sock.msg.server){
                  //sock.msg.server.close()
                  sock.msg.server.disconnect(true);
                }
                if(sock.msg.client){
                  sock.msg.client.close()
                }
                if(sock.msg.port){
                  options.proxyPorts.unshift(sock.msg.port)
                }
              }
            }
            delete sockList[data.requirements.serial.value]
          //}
        })
        ...
        .on('shell.command', function(channel, responseChannel, data) {
          log.info('receive shell.command:',data)
          joinChannel(responseChannel)
          push.send([
            channel
          , wireutil.transaction(
        ...
        .on('shell.keepalive', function(channel, data) {
          log.info('receive shell.keepalive:',data)
          push.send([
            channel
        ...
        .on('connect.start', function(channel, responseChannel) {
          log.info('receive connect.start')
          joinChannel(responseChannel)
          push.send([
            channel
          , wireutil.transaction(
        ...
        .on('screen.capture', function(channel, responseChannel,data) {
          joinChannel(responseChannel)
          push.send([
            channel
          , wireutil.transaction(
              responseChannel
            , new wire.ScreenCaptureMessage(data)
            )
          ])
        })
        ...
        .on('fs.pull',function(channel,responseChannel,data){
          log.info('websocket receive fs.pull',data)
          joinChannel(responseChannel)
          push.send([
            channel
            , wireutil.transaction(
              responseChannel
              , new wire.FileSystemPullMessage(data)
            )
          ])
        })
        .on('screen.response',function(channel,data){
          log.info('receive screen.response:',data);
          push.send([
            channel
            , wireutil.envelope(new wire.ScreenResponseMessage(data.sync))
          ])
        })
        .on('fs.appui',function(channel,responseChannel,data){
          log.info('receive fs.appui:',channel,responseChannel,data);
          joinChannel(responseChannel)
          push.send([
            channel
          ...
          push.send([
            channel
            , wireutil.transaction(responseChannel,new wire.DeviceAppUIGetMessage())
          ])
        })
        .on('script.debug',function(channel,responseChannel,data){
          log.info('receive script.debug',data);
          joinChannel(responseChannel)
          push.send([
            channel
            , wireutil.transaction(responseChannel,new wire.ScriptDebugMessage(data))
          ])
        })
        .on('script.stop',function(channel,responseChannel,data){
          log.info('receive script.stop',data);
        })
        .on('check.point',function(channel,responseChannel,data){
          log.info('receive check.point',data);
          joinChannel(responseChannel)
          push.send([
            channel
            , wireutil.transaction(responseChannel,new wire.PerKeyPointMessage())
          ])
        })
    })
    .finally(function() {
