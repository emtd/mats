  ...
  .then(function(){
        ...
        var child = proChild.exec(envStr + 'python ' + arg.scriptsPath, function (err, stdout, stderr) {
          ...
          fs.readdir(arg.capPath, function (err, files) {
            if (files && files.length > 0) {
              ...
              var path = arg.env.CAPTUREPATH.substring(arg.env.CAPTUREPATH.indexOf(splitStr) + splitStr.length)
              var pathList = '';
    ...
    function getLogName() {
      var path = {
        ...
        logName: arg.task + '_' + Date.now()+ '_' + 'tasklogs.json',
        perName: arg.task + '_' + Date.now()+ '_' + 'taskperformances.json'
      }
      return path;
    }
