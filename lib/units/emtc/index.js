                ...
                .spread(function (dbUser, dev) {
                  ...
                  joinGroup()
                    .then(function(){
                      ...
                      //启动appium,
                      var child=proChild.fork('./appium.js',[aport,bport,cport,sport,appiumPath,scriptsPath,dev.serial,dev.channel,dev.version,options.endpoints.push,taskId,capPath],{
                        cwd:__dirname
                      })
                      //脚本运行结束,释放端口.
          ...
          dbapi.loadDevice(data.serial)
            .then(function (device) {
              ...
              //释放设备
                ...
                if (responseDevice.usage=='automation') {//已经释放过了
                  returnObj.description = 'device ' + data.serial + ' is running script!'//has releaseed before,no need release again
                  return returnRes(returnObj)
                }
