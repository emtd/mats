            ...
            return new Promise(function (resolve, reject) {
              ...
              Promise.all([loadUser, loadDevice])//,mkCaptureDir
                .spread(function (dbUser, dev) {
                        ...
                        if(message.exist){
                          joinGroup()
                            .then(function(){
                              push.send([
                                dev.channel
                                , wireutil.envelope(new wire.ScriptStartMessage(JSON.stringify(data.env), data.spath, taskId))
                              ])
                            })
                        }
                        else{
                    ...
                    .on(wire.ScriptResultMessage, function (channel, message) {
                      ...
                      push.send([
                        ...
                        , wireutil.envelope(new wire.UngroupMessage(
                            wireutil.toDeviceRequirements({serial: { value: serial, match: 'exact' } } )
                          )
                        )
                      ])
                    })
                  ...
                  push.send([
                    dev.channel
                    , wireutil.envelope(new wire.ScriptCheckMessage(scriptName))
                  ])
