        ...
        function scriptRun() {
          ...
          Promise.map(taskList, function (item) {
            return new Promise(function (resolve, reject) {
              ...
              var aport=options.appiumPorts.shift();
              var bport=options.appiumPorts.shift();
              var logPath=pathutil.root('appium_log/'+dateutil.dateToStr()+dateutil.timeToStr()+'_'+serial);
              var loadAppium=appium.main({
                port:aport,
                bootstrap:bport,
                udid:serial,
                log:logPath,
              ...
              Promise.all([loadUser, loadDevice,loadAppium])
                .spread(function (dbUser, dev,loadApp) {
                  if (!dev) {
                    noExist.push(serial)
                    return resolve();
                  }
