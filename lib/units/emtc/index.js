        ...
        function scriptRun() {
          ...
          Promise.map(taskList, function (item) {
            return new Promise(function (resolve, reject) {
              ...
              var capPath = pathutil.logPath(dateutil.dateToStr(new Date()), serial, process.env.autoCapName)
                ...
                .spread(function (dbUser, dev) {
                  ...
                  joinGroup()
                    .then(function(){
                      ...
                      var scriptType = scriptsPath.substring(scriptsPath.lastIndexOf('.') + 1);//脚本类型
                      env.APPIUMPORT=aport;
                      //启动appium,
                      proChild.fork('./appium.js',[bport,cport,sport,appiumPath,scriptsPath,dev.serial,dev.channel,options.endpoints.push,taskId],{
                        cwd:__dirname,
                        env:{VERSION:dev.version,UDID:dev.serial,APPIUMPORT:aport,CAPTUREPATH:capPath,logName:process.env.logName,perName:process.env.perName,EMTCIP:process.env.EMTCIP,EMTCPORT:process.env.EMTCPORT}
                      })
                  ...
                  function disconnect(flag){
                    ...
                    push.send([
                      dev.channel
                      , wireutil.envelope(new wire.ScreenResponseMessage(ture))
                    ])
                    /*return new Promise(function (resolve, reject) {
                      var channelEvent = new events.EventEmitter();
                      var messageListener = wirerouter()
