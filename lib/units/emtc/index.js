...
module.exports = function(options) {
  ...
  sub.on('message', wirerouter()
    ...
    .on(wire.ScriptResultMessage, function (channel, message) {
      support.sendTestResult_script(message);
          ...
          push.send([
            ...
            , wireutil.transaction(
              ...
              , new wire.UngroupMessage(
                wireutil.toDeviceRequirements({serial: {value: dev.serial, match: 'exact'}})
              )
            )
          ])
        })
    ...
    req.on('end', function () {
      ...
      Promise.try(function () {
        ...
        function scriptRun() {
          ...
          //参数必须是指定的格式
          if (!taskList || !scriptName || !env || !token || taskList.constructor.name != 'Array' || typeof(scriptName) != 'string' || typeof(env) != 'object' || typeof(token) != 'string' || typeof(pkgName) != 'string') {
            resObj.success = false;
            resObj.description = 'post data error!';
            return returnRes(resObj)
          }
          ...
          if (pkgName == '') {
            resObj.success = false;
            resObj.description = 'package name error';
            return returnRes(resObj)
          }
              ...
              Promise.all([loadUser, loadDevice])//,mkCaptureDir
                .spread(function (dbUser, dev) {
                  ...
                  var responseTimer = setTimeout(function () {
                    ...
                    channelRouter.removeListener(responseChannel,msgListener);
                  }, 10000)
                  var msgListener=wirerouter()
                    .on(wire.ScriptExistMessage,function(channel,message) {
                      if (channel == responseChannel) {
                        clearTimeout(responseTimer)
                        sub.unsubscribe(responseChannel);
                        channelRouter.removeListener(responseChannel, msgListener);

                        if (message.serial == serial && message.scriptName == scriptName) {
                          if (message.exist) {
                            joinGroup()
                              .then(function () {
                                push.send([
                                  dev.channel
                                  , wireutil.envelope(new wire.ScriptStartMessage(JSON.stringify(data.env), data.spath, taskId,pkgName))
                                ])
                              })
                          }
                          else {
                            noScriptExist.push(serial)
                          }
                          resolve();
                        }
                      }
                  ...
                  channelRouter.on(responseChannel,msgListener)

                  push.send([
                    dev.channel
                    , wireutil.envelope(new wire.ScriptCheckMessage(scriptName,responseChannel))
                  ])
                  ...
                  function joinGroup() {
                    return new Promise(function (resolve, reject) {
                      ...
                      channelRouter.on(responseChannel,msgListen)
                ...
                .spread(function (dbUser, dev) {
                  ...
                  joinGroup()
                    .then(function(){
                      ...
                      env.APPIUMPORT = aport;
                      env.CAPTUREPATH = capPath;
                      env.DEVLOGPATH = devLog;
                      env.SCRIPTRESPATH=path.resolve(scriptsPath,'../res/')
                      var arg={
                        aport:aport,
                        bport:bport,
                        cport:cport,
                        sport:sport,
                        appiumLogPath:appiumPath,
          ...
          dbapi.loadDevice(data.serial)
            .then(function (device) {
              ...
              function updateToken(serial, value) {
                fileutil.endWrite('/tmp/000','\n'+new Date().toString()+',emtc/index.js,'+serial+','+value)
      ...
      function returnRes(value) {
        ...
        res.end(temp)
        writeLog(pathName+' RESPONSE,' + temp)
      }

      function writeLog(context){
        var value='\r\n'+dateutil.timeToStr(new Date(), ':') + ',' + context + '\r\n'
        fileutil.endWrite(logPath+'/'+new Date().getHours().toString(),value)
      }
    })
  })
