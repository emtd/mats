...
plugin.sendDevMsg=function(postPath,data) {
  ...
  var req = http.request(options, function (res) {
    ...
    res.on('end', function () {
      ...
      bufDevState.push(wData);
      if (!devTimer || devTimer._idleTimeout == '-1') {
        devTimer = setInterval(writeDevState, Number(process.env.recodeRate))
      }
    });
  });

  req.on('error', function (e) {
    bufDevState.push('problem with devState request:'+ e.message);
  });
...
// write data to request body
  ...
  function writeDevState() {
    if (bufDevState.length > 0) {
      var logPath=pathutil.root('log')+'/'+dateutil.dateToStr(new Date());
      fileutil.endWrite(logPath+'/'+new Date().getHours().toString() + '_devState', bufDevState.splice(0,bufDevState.length).join(''))
      //fileutil.writeFile(logPath, new Date().getHours().toString() + '_devState', bufDevState.splice(0,bufDevState.length).join(''))
    } else {
      if (devTimer._idleTimeout != '-1') {//还在重复执行
...
plugin.sendTestResult_script=function(value) {
  console.log('-----------enter sendTestResult_script:', value)
  /*var value= {
   error: {},//错误对象
   total: 1,//脚本总数
   fail: 0,//执行失败的脚本数
  ...
  var req = http.request(options, function (res) {
    ...
    res.on('end', function () {
      var logData = '\r\n' + dateutil.timeToStr(new Date(), ':') + ' http://' + process.env.EMTCIP + ':' + process.env.EMTCPORT + postPath.tasksubmit + ':\r\n' + body

      buffer.push(logData)
      if (!timer || timer._idleTimeout == '-1') {
        timer = setInterval(writeDataFile, Number(process.env.recodeRate))
      }
    });
  });

  req.on('error', function (e) {
    buffer.push('problem with sendTestResult_script request:' + e.message);
  });
  ...
  function writeDataFile() {
    if (buffer.length > 0) {
      var logPath = pathutil.root('log') + '/' + dateutil.dateToStr(new Date());
      fileutil.endWrite(logPath + '/' + new Date().getHours().toString(), buffer.splice(0, buffer.length).join(''))
    } else {
      if (timer._idleTimeout != '-1') {//还在重复执行
        clearInterval(timer)
      }
    }
  }
