...
module.exports = function(options) {
  ...
  var appiumPorts=options.appiumPorts.slice(0,options.appiumPorts.length)
  ...
  client.trackDevices().then(function(tracker) {
    ...
    function filterDevice(listener) {
      return function(device) {
        ...
        if(device.type.indexOf('no permissions')!=-1)
          device.type='nopermissions'
    ...
    tracker.on('add', filterDevice(function(device) {
      ...
      function spawn() {
        ...
        var appiPorts=appiumPorts.splice(0,6);
        var proc = options.fork(device, allocatedPorts.slice(),appiPorts)
        ...
        return resolver.promise
          ...
          .finally(function() {
            ...
            //Array.prototype.push.apply(appiumPorts, appiPorts)
            Array.prototype.unshift.apply(ports, allocatedPorts)
            Array.prototype.unshift.apply(appiumPorts, appiPorts)
            setTimeout(function(){
              console.log(appiPorts,'------------------------------',appiumPorts.length)
            },3000)
