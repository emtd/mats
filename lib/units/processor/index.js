...
module.exports = db.ensureConnectivity(function(options) {
  ...
  devDealer.on('message', wirerouter()
    ...
    .on(wire.DeviceAbsentMessage, function(channel, message, data) {
      ...
      dbapi.setDeviceDisUrl(message.serial,'')
      dbapi.setDeviceMsgProxyUrl(message.serial,'')
      appDealer.send([channel, data])
    })
    ...
    .on(wire.PerformanceEntryMessage,function(channel,messaage,data){
      appDealer.send([channel, data])
    })
    .on(wire.ScriptAutoEndMessage,function(channel,message,data) {
      dbapi.unsetDeviceUsage(message.serial)
      appDealer.send([channel, data])
    })
    .on(wire.ScriptResultMessage,function(channel,message,data) {
      appDealer.send([channel, data])
    })
    .on(wire.ScriptExistMessage,function(channel,message,data) {
      appDealer.send([channel, data])
    })
