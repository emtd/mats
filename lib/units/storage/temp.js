...
module.exports = function(options) {
  ...
  app.post('/s/upload/:plugin', function(req, res) {
    var form = new formidable.IncomingForm()//文件会在数据传递完成后生成
    ...
    Promise.promisify(form.parse, form)(req)
      .spread(function(fields, files) {
        return new Promise(function (resolve, reject) {
          var capPath = null;
          var imgName=fields.name
          if (fields.path) {
            capPath = pathutil.data(fields.path)//pathutil.logPath(dateutil.dateToStr(new Date()), fields.serial, process.env.capName)
          ...
          function genFileList() {
            return Object.keys(files).map(function (field) {
              ...
              if (file.type == 'image/jpeg') {
                if(capPath[capPath.length-1]!='/'){//末尾有/则不处理，没有则加/
                  imgName='/'+imgName;
                }
                newPath = capPath + imgName
      ...
      .then(function () {
        Promise.promisify(form.parse, form)(req)
          .spread(function (fields, files) {
            ...
            if(envStr!=''){
              fs.writeFile(path.resolve(debugPath,'../env'),envStr,function(err){
                if(err){
                  log.error('write env error:',err)
                }
              })
            }
            ...
            var readF=new Promise(function(resolve,reject) {
              fs.readFile(dir + '/' + item, function (err, data) {
                if(err){
                  ...
                  resolve({name:item,img:data.toString('base64')})
                }
              })
            })
            proList.push(readF);
