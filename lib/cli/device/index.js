...
module.exports.builder = function(yargs) {
  return yargs
    ...
    .option('aport', {
      describe: 'device appium port to use.'
      , type: 'number'
      , default: 10000
    })
    .option('bport', {
      describe: 'device appium bootstrap port to use.'
      , type: 'number'
      , default: 10000
    })
    .option('cport', {
      describe: 'device appium chromedriver port to use.'
      , type: 'number'
      , default:10000
    })
    .option('sport', {
      describe: 'device workers appium selendroid port to use.'
      , type: 'number'
      , default:10000
    })
    .option('msg-port', {
      describe: 'Port allocated to the msg WebSocket.'
      , type: 'number'
      , demand: true
    })
    ...
    .option('appium-http-port', {
      describe: 'Port allocated to the appium log http WebSocket.'
      , type: 'number'
      , demand: true
    })
    .option('secret', {
...
module.exports.handler = function(argv) {
  ...
  , msgWsUrlPattern: argv.msgWsUrlPattern
  , appiumHttpPort:argv.appiumHttpPort
  , ssid:argv.ssid
  , secret:argv.secret
    //结束
