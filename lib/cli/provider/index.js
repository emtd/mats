...
module.exports.builder = function(yargs) {
  ...
  return yargs
    ...
    .option('max-appium-port', {
      describe: 'Highest port number for device workers appium to use.'
      , type: 'number'
      , default: process.env.DMAXAPPIUMPORT||12000
    })
    .option('min-appium-port', {
      describe: 'Lowest port number for device workers appium to use.'
      , type: 'number'
      , default: process.env.DMINAPPIUMPORT||10000
    })
    .option('msg-ws-url-pattern', {
      describe: 'The URL pattern to use for the msg WebSocket.'
      , type: 'string'
      , default: 'http://${publicIp}:${publicPort}' //'ws://${publicIp}:${publicPort}'
    })
...
module.exports.handler = function(argv) {
  ...
  , appiumPorts: range(argv.minAppiumPort, argv.maxAppiumPort)
  , allowRemote: argv.allowRemote
  , fork: function(device, ports,appiumPorts) {
      ...
      , '--vnc-port', ports.shift()
        //自定义 开始
      , '--aport', appiumPorts.shift()//appium
      , '--bport', appiumPorts.shift()//-bp Appium bootstrap端口
      , '--cport', appiumPorts.shift()//--chromedriver-port chromedriver端口（当使用了webviews或者chrome）
      , '--sport', appiumPorts.shift()//--selendroid-port selendroid端口（当使用了selendroid）
      , '--msg-port', appiumPorts.shift()
      , '--msg-ws-url-pattern', argv.msgWsUrlPattern
      , '--appium-http-port',appiumPorts.shift()
      , '--ssid',process.env.SSID || 'ssid'
