...
module.exports.handler = function(argv) {
  ...
  , fork: function(device, ports) {
      ...
      , '--vnc-port', ports.shift()
        //自定义 开始
      , '--aport', ports.shift()//appium
      , '--bport', ports.shift()//-bp Appium bootstrap端口
      , '--cport', ports.shift()//--chromedriver-port chromedriver端口（当使用了webviews或者chrome）
      , '--sport', ports.shift()//--selendroid-port selendroid端口（当使用了selendroid）
      , '--msg-port', ports.shift()
      , '--msg-ws-url-pattern', argv.msgWsUrlPattern
      , '--appium-http-port',ports.shift()
      , '--ssid',process.env.SSID || 'ssid'
      , '--secret',process.env.SECRET
      , '--proxy-mode',argv.proxyMode
        //自定义 结束
