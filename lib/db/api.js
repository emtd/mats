...
dbapi.saveDeviceIdentity = function(serial, identity) {
  ...
  , msgUrl:identity.msgUrl
  , proxyMsgUrl:identity.proxyMsgUrl
  }))
}
...
//lgl
dbapi.setDeviceToken=function(serial,token){
  return db.run(r.table('devices').get(serial).update({
    token: token
  }))
}

dbapi.setDeviceDisUrl=function(serial,url){
  return db.run(r.table('devices').get(serial))
    .then(function(dev){
      var display=dev.display;
      display.proxyUrl=url
...
dbapi.setDeviceMsgProxyUrl=function(serial,url) {
  return db.run(r.table('devices').get(serial).update({
    proxyMsgUrl: url
  }))
}
